...
static const char *TAG = "dhtMeasurementTask";
...
static bool dht_await_pin_state(uint8_t pin, int64_t timeout, bool expected_pin_state, int64_t *duration)
{
    int64_t ts;

    ts = esp_timer_get_time();
    while (ts + timeout >= esp_timer_get_time())
    {
        if (gpio_get_level(pin) == expected_pin_state)
        {
            if (duration)
            {
                *duration = esp_timer_get_time() - ts;
            }
            return true;
        }
    }
